% - Lead-DBS Job created on 14-Aug-2022 10:18:23 -
% --------------------------------------
...
% read options for IPN243 (left electrode only), Medtronic 3389 and St. Jude 6172
% electrodes respectively
options_ipn243 = getoptslocal_ipn243;
options_medtronic = getoptslocal_medtronic;
options_stjude = getoptslocal_stjude;

% successively run each pre-reconstruction for IPN243, Medtronic and St. Jude
ea_run('run', options_ipn243);
ea_run('run', options_medtronic);
ea_run('run', options_stjude);
...
message = sprintf('%s: %s %s %s',date,'Pre-reconstruction (via PaCER) of eleven patients took',string(sec),'(hh:mm:ss).');
...
% prepare options for each of the runs (IPN243, Medtronic, St. Jude)
% IPN243
function options = getoptslocal_ipn243
    ...
    options.uipatdirs = {'/Volumes/Samsung_T5/mercenaries/dbs_speech_neurons/imaging_data/IPN243'};
...
% Medtronic
function options = getoptslocal_medtronic
    options.endtolerance = 10;
    options.sprungwert = 4;
    options.refinesteps = 0;
    options.tra_stdfactor = 0.9;
...
% St. Jude
function options = getoptslocal_stjude
    options.endtolerance = 10;
    options.sprungwert = 4;
    options.refinesteps = 0;
    options.tra_stdfactor = 0.9;
    ...
    options.writeoutpm = 1;
    options.elmodeln = 12;
    options.elmodel = 'St. Jude Directed 6172 (short)';
    options.atlasset = 'DISTAL Minimal (Ewert 2017)';
    options.atlassetn = 21;
    options.reconmethod = 'PaCER (Husch 2017)';
    options.expstatvat.do = 0;
